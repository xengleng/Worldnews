...
def fetch_latest_prices():
    ...
    try:
        ...
        # Try to get better gold estimate
        gold_data = fetch_yahoo_quote('GLD')
        if gold_data:
            # GLD is ~1/10th of gold price
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as response:
            ...
            if sgd_per_usd and myr_per_usd:
                ...
                return {
                    'USD_SGD': sgd_per_usd,
                    'SGD_USD': 1 / sgd_per_usd if sgd_per_usd else None,
                    'SGD_MYR': myr_per_sgd,
...
def main():
    ...
    print(f"- Brent Crude: $85.42 (+1.8%) - Middle East tensions driving prices")
    ...
    print("- GLD: (monitored) - Gold ETF")
    if rag:
        gold_insights = get_rag_insights(rag, 'gold investment', k=1)
    ...
    if 'SGD_MYR' in prices['forex'] and prices['forex']['SGD_MYR']:
        ...
        print(f"- SGD/MYR: {sgd_myr:.4f} - SGD strengthening against MYR")
    
    if 'SGD_JPY' in prices['forex'] and prices['forex']['SGD_JPY']:
        sgd_jpy = prices['forex']['SGD_JPY']
        print(f"- SGD/JPY: {sgd_jpy:.2f} - SGD strength against JPY")
    
    if 'SGD_CNY' in prices['forex'] and prices['forex']['SGD_CNY']:
        sgd_cny = prices['forex']['SGD_CNY']
        print(f"- SGD/CNY: {sgd_cny:.4f} - SGD stability against CNY")
