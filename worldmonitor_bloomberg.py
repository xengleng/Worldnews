#!/usr/bin/env python3
"""
WorldMonitor with Bloomberg-style categorization and alerts.
Combines news aggregation with professional financial news categorization.
"""
...
def format_bloomberg_report(articles):
    ...
    lines = []
    lines.append(f"World Monitor Brief - {datetime.now().strftime('%Y-%m-%d %H:%M')} SGT")
    lines.append(f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M')} SGT")
    lines.append("")
    ...
    for level in [4, 3, 2, 1]:
        if alerts_by_level[level]:
            ...
            lines.append(f"{alert_info['color']} {alert_info['name']} ALERTS ({len(alerts_by_level[level])}) {alert_info['description']}")
            
            for i, article in enumerate(alerts_by_level[level], 1):
                lines.append(f"{i}. {article['title']} [{article['bloomberg_category']} → {article['bloomberg_subcategory']}] Source: {article['source']}")
...
def main():
    """Main function."""
    print("📈 WorldMonitor with Bloomberg Categorization")
    print("=" * 50)
